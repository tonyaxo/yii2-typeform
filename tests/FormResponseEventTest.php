...
use tonyaxo\yii2typeform\api\webhooks\ResultField;
use tonyaxo\yii2typeform\api\webhooks\FormResponseEvent;

class FormResponseEventTest extends \yiiunit\extensions\yii2typeform\TestCase
{
    /**
    ...
    public function testResults()
    {
        ...
        $request->expects($this->any())
            ->method('getRawBody')
            ->willReturn($this->webhookData());

        /** @var FormResponseEvent $result */
        $result = WebhookEvent::handle($request);

        $this->assertInstanceOf(FormResponseEvent::class, $result);
        $results = $result->getResults();

        // text
        $this->assertResult($results,  'DlXFaesGBpoF', 'It\'s cold right now! I live in an older medium-sized city with a university. Geographically, the area is hilly.');
        // payment
        $this->assertResult($results,  'pn48RmPazVdM', 'Successful payment, from John Doe, card 1234, total 1');
        // email
        $this->assertResult($results,  'SMEUb7VJz92Q', 'laura@example.com');
        // text
        $this->assertResult($results,  'JwWggjAKtOkA', 'Laura');
        // date
        $this->assertResult($results,  'KoJxDM3c6x8h', '2005-10-15');
        // choices
        $this->assertResult($results,  'PNe8ZKBK8C2Q', 'London,Sydney');
        // number
        $this->assertResult($results,  'Q7M2XAwY04dW', '5');
        // boolean
        $this->assertResult($results,  'gFFf3xAkJKsr', 'Yes');
        // choice
        $this->assertResult($results,  'k6TP9oLGgHjl', 'London');
        // boolean
        $this->assertResult($results,  'RUqkXSeXBXSd', 'No');
    }

    protected function assertResult(array $results, $id, $answer): void
    {
        $this->assertArrayHasKey($id, $results);
        /** @var ResultField $field */
        $field = $results[$id];
        $this->assertInstanceOf(ResultField::class, $field);
        $this->assertEquals($answer, $field->getAnswerText());
    }

    /**
     * @return string
     */
    protected function webhookData(): string
    {
        return <<<JSON
{
    "event_id": "LtWXD3crgy",
